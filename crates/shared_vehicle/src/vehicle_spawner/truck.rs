...
use bevy_rapier3d::{ prelude::{ Collider, ColliderMassProperties, CollisionGroups, ComputedColliderShape, Friction, Group, MassProperties, RigidBody}, rapier};
use crate::accessory_controls::truck::{assets::update_truck_control_mapping, controls::TruckMeshMapping};

use super::{follow::CopyPosition, react_on_scene_instance_ready::{OnSceneReady, ReactOnSceneInstanceReady}, scoop::ScoopTarget, VehicleType};
...
pub fn spawn_truck<'a>(
    ...
    let truck =
    assets.load(GltfAssetLabel::Scene(0).from_asset("private/truck/truck.gltf"));
    ...
    let meshes_to_convert_to_collider: HashMap<String, Option<ComputedColliderShape>> = [
        // main dump
        (main_dump_name.to_string(), Some(ComputedColliderShape::default())),
    ].into();
    // Model
    entity.with_child((
        ...
        | {
            ...
            for entity in q_children.iter_descendants(trigger.entity()) {
                ...
                commands.entity(entity).insert(CollisionGroups::new(Group::GROUP_4, !Group::GROUP_4));
